...
1. **Node:** A node represents a "real-world thing" on Facebook like a user or a page. In the Facebook PHP SDK, nodes that are returned from a Graph response are represented as `GraphNode`'s.
2. **Edge:** An edge is the relationship between two or more nodes. For example a "photo" node would have a "comments" edge. In the Facebook PHP SDK, edges returned from a Graph response are represented as a collection of `GraphNode`'s called a `GraphEdge`.
